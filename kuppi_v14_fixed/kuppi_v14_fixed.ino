...
void loop() {
  // if(wifiStatus != WIFI_OFFLINE && millis() - wifiStatusTime > WIFI_STATUS_RESET_MS){
  //   wifiStatus = (WiFi.status() == WL_CONNECTED) ? WIFI_OK : WIFI_OFFLINE;
  //   drawWifiDot();
  // }

  // if(timerRunning && millis() - lastBorderTime > 1000){
  //   lastBorderTime = millis();
  //   drawTimerBorder();
  // }
  ...
  // NFC — paused during checklist so touch has exclusive I2C access
  if(currentScreen != SCREEN_CHECKLIST && millis() - lastNFCTime > 300){
    lastNFCTime = millis();
    uint8_t uid[7];
    uint8_t uidLen = 0;
    bool found = false;
